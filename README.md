...
```python
engineConnect.exportApp(dir = path_to_export, onlyPublished = False, idInNaming = True, streamId = streamId, appId = appId, createFolderStructre = True, idInNaming = True)
```
вывод в консоль:
```sh
...
...
- **onlyPublished** - Принимает значения True/False. Указывает на выгрузку только опубликованных приложений(True) или всех(False). **False по умолчанию**
